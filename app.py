...
def directs(direct):
    ...
    max_nights = 0
    for i in tours:
        if tours[i]["departure"] == direct:
            dir_extract.append(i)
            if tours[i]["price"] > max_price:
                max_price = tours[i]["price"]
            if tours[i]["price"] < min_price:
                min_price = tours[i]["price"]
            if tours[i]["nights"] > max_nights:
                max_nights = tours[i]["nights"]
            if tours[i]["nights"] < min_nights:
                min_nights = tours[i]["nights"]
    count = len(dir_extract)
    return render_template("direction.html", tours=tours, direct=direct, count=count, min_price=min_price, max_price=max_price, max_nights=max_nights, min_nights=min_nights, dir_extract=dir_extract)
